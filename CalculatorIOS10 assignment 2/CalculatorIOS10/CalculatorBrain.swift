  ...
  /* private func setHaakjes(descriptionString: String) -> String{
    ...
    mutating func performOperation(_ symbol: String){
        if let operation = operations[symbol] {
            ...
            case .binaryOperation(let function, let descriptionBin):
                //altijd als het
//                if firstToExecute{
...
//            }
    ...
    mutating func setOperand(_ operand: Double){
        accumulator = operand
        stackOfCalcObjects.append(CalcObjects.operand(operand))
       // print(" operand is: ", descriptionAccumulator!)
    }
    var variableOperands: Dictperionary<String, Double>?
    mutating func setOperand(variable named: String){
        //give default value to "named" variable
        stackOfCalcObjects.append(CalcObjects.variable(named))
         
    }
    func evaluate(using variables: Dictionary<String,Double>? = nil) -> (result: Double?, isPending: Bool, description: String) {
     
        if variableOperands != nil{
            if let myInternalAccumulator  = variables![variableOperands!.keys.first!]{
                print(String(myInternalAccumulator))
            }
        }
        ...
        return (calculatorResult!.result, calculatorResult!.isPending, calculatorResult!.description)
    }
    
    private var stackOfCalcObjects = [CalcObjects]()
    private enum CalcObjects {
        case operation(String)
        case operand(Double)
        case variable(String)
        
    }
    private var calculatorResult : CalculatorResult?
